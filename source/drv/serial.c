...
int serial_available(void) { return uart_getreg(UART_LSR) & 0x01; }
...
static void usb_get_handle_irqs() {
    ...
    if (r6 & USB_CTRL_INTRUSB_RESET) {
        ...
        writeb(USB_CTRL_INTRUSBE_SUSPEND_ENABLE | USB_CTRL_INTRUSBE_RESUME_ENABLE |
                       USB_CTRL_INTRUSBE_RESET_ENABLE,
               USB_CTRL_INTRUSBE);
        writeb(readb(USB_CTRL_INTRINE) | USB_CTRL_INTRINE_EP0_ENABLE,
               USB_CTRL_INTRINE);
    }
